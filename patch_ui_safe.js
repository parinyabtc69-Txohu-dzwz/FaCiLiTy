...
const oldRenderRepair = `window.renderRepairTable = function () {
  const rawData = window.allRepairTasks || [];
  const listWithIndex = rawData.map((r, idx) => ({ r, originalIndex: idx }));

  const getWeight = st => {
    const s = (st || '').trim();
    if (['เสร็จสิ้น', 'เสร็จ/คืนเรียบร้อย', 'เรียบร้อยแล้ว'].includes(s)) return 3;
  ...
  if (filteredList.length === 0) {
    tbody.innerHTML = '<tr class="bg-white"><td colspan="7" class="p-8 text-center text-slate-500">ไม่มีรายการแจ้งซ่อมที่ตรงกับเงื่อนไข</td></tr>';
    return;
  }
...
if (content.includes(oldRenderRepair)) {
  content = content.replace(oldRenderRepair, newRenderRepair);
} else {
  console.log('WARNING: oldRenderRepair not found');
}

// 2. Add detailsHtml to window.updateAdvTask
const oldUpdateAdvTask = `window.updateAdvTask = async function (type, index, currentStatus) {
...
const newUpdateAdvTask = `window.updateAdvTask = async function (type, index, currentStatus) {
...
if (content.includes(oldUpdateAdvTask)) {
  content = content.replace(oldUpdateAdvTask, newUpdateAdvTask);
} else {
  console.log('WARNING: oldUpdateAdvTask not found');
}
