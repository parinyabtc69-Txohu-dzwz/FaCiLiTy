  ...
  liff.getProfile().then(profile => {
    ...
    post({ action: 'link_line_account', email: email, lineId: lineId, name: name, picture: picture })
      .then((res) => {
        ...
        sessionStorage.setItem('logged_email', currentEmail);
  ...
  tbody.innerHTML = filteredList.map(({ r, originalIndex }) => {
    ...
    if (isDone) {
      ...
      if (rating > 0) {
        ...
        firstColHtml = `<button onclick="openSurveyModal('repair', ${originalIndex})" class="text-xs text-blue-500 hover:text-blue-700 font-semibold whitespace-nowrap"><i class="fa-regular fa-star mr-0.5"></i>ประเมิน</button>`;
      }
    } else {
      if (newUrgency === 'ด่วน' || detailStr.includes('ความเร่งด่วน: ด่วน')) {
