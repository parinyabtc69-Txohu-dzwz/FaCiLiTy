...
window.renderRepairTable = function () {
  ...
  tbody.innerHTML = filteredList.map(({ r, originalIndex }) => {
    ...
    const activeStarHtml = '<i class="fa-solid fa-star text-base animate-bounce"></i>';
    const activeStars = Array(urgencyLevel).fill(activeStarHtml).join('');
    
    const inactiveStarHtml = '<i class="fa-regular fa-star"></i>';
    const inactiveStars = Array(urgencyLevel).fill(inactiveStarHtml).join('');

    const starIcon = isDone
      ? `<span class="text-slate-300 flex justify-center gap-1">${inactiveStars}</span>`
      : `<span class="text-amber-400 drop-shadow-sm flex justify-center gap-1">${activeStars}</span>`;
